...
func NewLogger(prefix string) (*Logger, error) {
	...
	ErrorName := fmt.Sprintf("%s/%s_error.log", folderName, prefix)
	errorFile, err := os.OpenFile(ErrorName, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
	if err != nil {
		return nil, fmt.Errorf("failed to open general log file: %w", err)
	}
	DebuggerConsole := io.MultiWriter(debugFile, os.Stdout)
	// Create loggers for each purpose
	errorLogger := log.New(errorFile, "ERROR: ", log.Ldate|log.Ltime|log.Lshortfile)
	infoLogger := log.New(generalLogFile, "INFO: ", log.Ldate|log.Ltime)
	debugLogger := log.New(DebuggerConsole, "DEBUG: ", log.Ldate|log.Ltime|log.Lshortfile)
...
func NewScraper(c *colly.Collector, s *colly.Collector, l *Logger, cache Cache) *scrape {
	return &scrape{
		mainScraper: c,
		sideScraper: s,
		logger:      l,
...
func initScrape() (*scrape, error) {
	...
	configColly(mainPage, log, cache)
	configColly(sidePage, log, cache)
	return NewScraper(mainPage, sidePage, log, cache), nil
}
func Config() *scrape {
	c, err := initScrape()
	if err != nil {
...
func (s *scrape) BeginScrape(ctx context.Context, link chan string) {
	s.logger.DebugLogger.Println("Parsing main pages")
...
func (s *scrape) ScrapeSidePages(ctx context.Context, source chan string) {
	...
	c := s.sideScraper
	db := DB.GetStorage()
	s.logger.DebugLogger.Println("Scraping side pages")
	c.OnHTML("body", func(h *colly.HTMLElement) {
		...
		lat, long := GeoPoints(location)
		id := db.AddEvent(event)
		db.AddGeoPoint(id, DB.NewGeoPoint(lat, long, location, title))
	})
	//var counter int
	//var mu sync.Mutex
	var wg sync.WaitGroup
	...
	for i := 0; i < workerPool; i++ {
		...
		go func() {
			...
			s.logger.DebugLogger.Println("Worker exiting")
		}()
	}
}
...
func configColly(c *colly.Collector, log *Logger, cache Cache) error {
	...
	c.OnError(func(r *colly.Response, err error) {
		...
		if r.StatusCode != 404 {
			...
			cache.IncreaseTTL(url, time.Hour*24*7*52) // 1 year
			// if the url doesnt exist place it in never touch territory
		}

		if len(r.Body) > 0 {
			log.ErrorLogger.Printf("Response Body: %s\n", string(r.Body[:100])) // first 100 bytes
		}
	})
	return nil
}
